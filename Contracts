...
contract BraveToken is Context, IBEP20, Ownable {
    ...
    function lotteryOpen(uint256 amount) private {
        ...
        if(_lotteryIncluded.length > 0) {
            for(uint256 i; i < whiteList.wonNum(); i++) {
        ...
        if(claimLotteryCount >= whiteList.claimLotteryNumber()) {
            lotteryTermsAmount.pop();
            lotteryTermsAmount.push(termLotteryBNB.div(whiteList.getLotteryTermWonNum()));
            lotteryTerm += 1;
            lotteryTermsAmount.push(0);
            claimLotteryCount = 0;
            termLotteryBNB = 0;
    ...
    function paySwapTxFee(uint256 amount) private {
        ...
        lotteryPoolBNB += txFeeBase;
        termLotteryBNB += txFeeBase;
        rewardPoolBNB += restFee;
        if(_lotteryIncluded.length == 0) {
            (sent,) = salePool.call{value : txFeeBase.mul(2)}(""); 
        }else {
            address rewardAddress =  _lotteryIncluded[BraveCommon.random(0, _lotteryIncluded.length, amount)];
            (sent,) = rewardAddress.call{value : txFeeBase}("");
            require(sent);
            (sent,) = salePool.call{value : txFeeBase}("");
        }
        require(sent);
    }
